# PRD — Extraction Flows ("Distillations")
...
that volume. This is a *distillation* of many documents down to structured
records — a second, parallel flow paradigm, not a feature bolted onto chat.

## 2. Users / Personas

- **Procurement / HR / Ops lead (author)** — designs a distillation: what fields
  to pull, how to read the inputs, what the output document/spreadsheet looks
  like. Non-technical; no code, no prompt engineering.
- **Operator (runner)** — uploads the document set, runs the distillation,
  previews quality, and reviews/finalises the results.
- **Admin** — turns the feature on (per role), sees all distillations across the
  org, and governs cost and retention.

## 3. Goals

- An author can create a **Distillation** (extraction flow) with an ordered,
  typed field schema and plain-English AI instructions — reusing existing flow
  authoring, versioning, publishing, audit, and auth.
- An operator can upload documents and **zips** (folder structure preserved),
  run a distillation over hundreds of documents asynchronously, and see live
  progress with a preview breakpoint.
...
| `ExtractionInputConfig` | `entities/extraction-schema.ts` | new | cardinality (`one_per_file` \| `many_per_record`) + free-text guidance. |
...
| `ExtractionRecord` | `entities/extraction-record.ts` | new | one per output record; `fields[]`, aggregate confidence, `sourceDocumentIds`. |
| `PermissionKey` (+2) | `entities/permission.ts` | change | `distillation:author`, `distillation:run`. |

## 6. User stories

1. As an **author**, I can create a distillation and edit it as **two large
...
2. As an **author**, on the **input card** I can set instructions for how the AI
   ...
   **many-files-per-record**, add free-text detail, and upload zips/documents
   into a **folder tree** that preserves structure.
...
4. As an **operator**, I can run a distillation with **preview on by default**
...
8. As a **user**, I reach the feature via a **"Distillations"** menu item,
   separated from **Flows** by a subtle horizontal line; extraction flows never
   appear in my Flows list or New Chat.
9. As an **admin**, I can enable the feature per role, and see an **All
   Distillations** page mirroring the user list.

## 7. Pages / surfaces affected

- Sidebar (`components/sidebar.tsx`) — new **Distillations** item (user + admin);
  user side gets an `<hr>` separator between Flows and Distillations. Item shown
  only when the `distillations` flag resolves for the user.
...
- New Chat modal (`components/chat/new-chat-modal.tsx`) — excludes extraction
  flows.
- `/distillations` — user list: each distillation row has two sub-rows (most
  recent run, or "not yet run"); "show more" → all runs, paginated 20/page.
- `/distillations/[id]/edit` — the two-card editor.
- `/distillations/[id]/runs/[runId]` — run progress + results viewer.
- `/admin/distillations` — admin "All Distillations" list (mirrors user list).
- tRPC: new `extraction` router (`createSchema`, `runSample`, `startBatch`,
  `cancel`, `retryFailed`, `runStatus`, `generateDocuments`, `export`,
  `editResult`, `runReport`).
...
permission tables — the `distillations` flag reuses `core_feature_flag` +
`admin_feature_flag_roles` (ADR-022); the two permission keys extend the
developer-owned `PERMISSIONS` registry (ADR-021).

## 9. Architectural decisions

- **ADR-033 — Extraction Flows ("Distillations"): a second flow paradigm** (new,
  introduced by this PRD). Establishes the `flow_type` discriminator, the
  snapshot-union authoring model, record cardinality (input file ≠ output
  record), the three-table results model, the ADR-019-based batch worker, and the
  flag + permission gating.
...
- [ ] The feature (menu + routes + procedures) is invisible and inert unless the
      `distillations` flag resolves for the user; authoring vs running are gated
      by `distillation:author` / `distillation:run`, server-enforced.
- [ ] Extraction flows never appear in the user Flows list, admin Flows list, or
      New Chat — enforced at the query layer.
- [ ] "Distillations" appears in the user menu under a subtle `<hr>` separator
      below Flows, and as "All Distillations" in admin.
- [ ] The distillations list shows two sub-rows per row (latest run / not-yet-run)
      with "show more" → all runs paginated 20/page.
- [ ] The editor renders as two cards (input → output) with an arrow between; the
      ...
      closed) and offers the cardinality toggle + free-text guidance; the output
      card offers docx/xlsx, context-doc upload, and a summary toggle + template.
...
- Percentage rollout for the `distillations` flag (`rollout_pct` remains inert,
  per ADR-022).
...
- **Many-files→one-record grouping key** — first-level folder is the proposed
  grouping; confirm behaviour for flat uploads and mixed structures at build.
