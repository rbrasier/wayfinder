...
Every published tag is immutable — `0.28.13` is always the same image — so an
...
docker run --rm -e DATABASE_URL="postgresql://…" \
  ghcr.io/rbrasier/wayfinder:0.28.13 migrate
```
...
sed -i 's/^WAYFINDER_VERSION=.*/WAYFINDER_VERSION=0.28.13/' .env
...
- **Read the release notes** for the version you are moving to, and for any you
  ...
  from `0.26.x` to `0.28.13` means you are taking `0.27.x`'s migrations too.
