...
Every integration variable is an **optional override** — the app reads the DB
first and falls back to these. Common ones:

| Variable | Value |
|---|---|
| `DATABASE_URL` | `postgresql://postgres:postgres@localhost:5433/wayfinder` (matches the docker-compose Postgres host port) |
| `REDIS_URL` | `redis://localhost:6379` |
| `BETTER_AUTH_SECRET` | Auto-generated by `restart.sh`; set only to pin a value |
